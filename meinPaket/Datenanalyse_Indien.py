...
df = pd.read_csv(r".\Data\Economy_Productivity_SD_India.csv")
df2 = pd.read_csv(r".\Data\GDP_Sector_Income_2019_2023_30Cities.csv")
df = pd.DataFrame(df)
df2 = pd.DataFrame(df2)

class Datenanalyse():
    '''Klasse zur darstellung der Daten'''
    def __init__(self,X=1,N=4):
        self.X = X
        self.N = N
    
    def mosaic_plot(self,dataframe):
        '''Plottet das Mosaikplot von plotSectors'''
        props = {}
    ...
    def plotSectors(self):
        '''Bringt die Daten in ein Format um es per mosaic_plot zu plotten.'''
        X = self.X - 1
        N = self.N
    ...
    def check_and_adjust_values(self, value1, value2): #100% chatgpt
        '''Insofern tick label zu nah aneinander sind gibt diese Funktion True aus'''
    ...
    def plotDevelopment(self,plotNummer=1): #1-30
        '''Plot Funktion für 7 Plots zur entwicklung verschiedener bereiche der Städte'''
        anfang=self.X
        ende=self.N
        ...
        for i in range(len(städte)):
            ...
            aktuelleStadt = aktuelleStadt.iloc[:,plotNummer+1].reset_index(drop=True) #nur die infos fürs gewollte plot auswählen
